...
from ..blocks import ArrayBlock, BlockCollection, ChromatogramBlock, ExtensionBlock, GlobalMetaBlock, PrecursorBlock, RunBlock, SpectrumBlock
...
class RealMzmlParseTool(BaseBlockTool):
    ...
    def build_blocks(self, context: PipelineContext) -> None:
        ...
        candidate = _build_candidate(document, digest)
        _validate_candidate(candidate)
        context.blocks = candidate


def _build_candidate(document: ParsedMzmlDocument, digest: str) -> BlockCollection:
    ...
    for position, item in enumerate(document.spectra, 1):
        ...
        if item.ms_level == 2:
            ...
            if (
                ...
                raise MzmlParseError(
                    "INCOMPLETE_ADMITTED_PRECURSOR",
                    "admitted MS2 precursor lacks one selected ion with m/z, nonzero charge, and intensity",
        ...
        arrays.extend(
            (
                ArrayBlock(mz_array_id, "mz", "float64", list(item.mz_values)),
                ArrayBlock(intensity_array_id, "intensity", "float64", list(item.intensity_values)),
            )
        )

    auxiliary_records: list[AuxiliaryArrayV1] = []
    for position, item in enumerate(document.chromatograms, 1):
        ...
        arrays.extend(
            (
                ArrayBlock(time_array_id, "time", "float64", list(item.time_values_seconds)),
                ArrayBlock(intensity_array_id, "intensity", "float64", list(item.intensity_values)),
            )
        )
    ...
    return BlockCollection(
        global_meta=GlobalMetaBlock(
            ...
            notes=["P1-B5 real mzML conversion: strict single-run centroid MS1/MS2 plus TIC/BPC subset."],
        ),
        runs=[
            RunBlock(
                run_id=document.run.run_id,
...
def _validate_candidate(candidate: BlockCollection) -> None:
    ...
    if len(extension_map) != len(candidate.extensions) or MZML_METADATA_EXTENSION_TYPE not in extension_map:
